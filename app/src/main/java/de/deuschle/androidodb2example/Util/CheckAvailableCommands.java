...
public class CheckAvailableCommands {
    ...
    public static HashMap<String, Boolean> getAvailabilityMap(List<String> PIDAvailabilityList) {
        return new CheckAvailableCommands().getInternalAvailabilityMap(PIDAvailabilityList, 0);
    }

    private HashMap<String, Boolean> getInternalAvailabilityMap(List<String> PIDAvailabilityList, int ID) {
        // Leerzeichen und die ersten zwei Bytes entfernen
        String currentPids;
        try {
            currentPids = PIDAvailabilityList.get(ID);
        } catch (IndexOutOfBoundsException e) {
            return availabilityMap;
        }
        String availableCommandsInput = currentPids.replaceAll(" ", "");
